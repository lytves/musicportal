...
$nam_e1 = 'ѕопул€рные поисковые запросы за неделю (по названию трека)';
...
$metatags['title'] = 'ѕопул€рные поисковые запросы за неделю (по названию трека) '.$metapage.'на '.$config['home_title_short'];
$metatags['description'] = 'ѕопул€рные поисковые запросы за неделю (по названию трека) на '.$config['description'];
...
$db->query( "SELECT search_text, COUNT(search_text) AS cnt, (SELECT COUNT(DISTINCT search_text) FROM ".PREFIX."_mservice_search WHERE (search_type = '3' AND count != '0' AND time_php > '$toptime')) as cnt2 FROM ".PREFIX."_mservice_search WHERE (search_type = '3' AND count != '0' AND time_php > '$toptime') GROUP BY search_text HAVING cnt > 0 ORDER BY cnt DESC LIMIT ".$limit."," . $mscfg['track_page_lim'] );

$mtitle = '<h3 class="tit">ѕопул€рные поисковые запросы за неделю (по названию трека)</h3>';

if ( $db->num_rows( ) == 0 ) $stop[] = 'Ќет треков дл€ этой страницы ѕопул€рных поисковых запросов за неделю (по названию трека)';
else {
	$mcontent = <<<HTML
<table width="100%">
<tr>
...
HTML;
	...
	while( $row = $db->get_row( ) ) {
		...
		$search_text_link = '<a href="'.$config['http_home_url'].'music/search-3-1-'.$search_text64.'.html" title="'.$row['search_text'].'">'.$search_text.'</a>';
