...
$nam_e1 = 'Топ скачиваний mp3 за неделю';
...
$metatags['title'] = 'Топ скачиваний mp3 за неделю '.$metapage.'на '.$config['home_title_short'];
$metatags['description'] = 'Топ скачиваний mp3 за неделю на '.$config['description'];
...
$mtitle = '<h3 class="tit">Топ скачиваний mp3 за неделю</h3>';

if ( $db->num_rows( ) == 0 ) $stop[] = 'Нет треков для этой страницы Топ скачиваний mp3 за неделю';
else {
	$mcontent = <<<HTML
...
<td align="center">'.formatsize( $row['size'] ).'</td>';
		...
		$mcontent .= '<td align="center"><a href="'.$t_link.'" title="Скачать '.$query_24hr[$i]['artist'].' - '.$query_24hr[$i]['title'].' mp3"><div class="download_tracklist"></div></a></td></tr>';
	}
	$mcontent .= '</table></div>';
	...
	for ( $t = 0; $count_d > $t; $t++ ) {
		$t2 = $t + 1;
		$plink = $config['http_home_url'].'music/downloadstopweek_admin-page-'.$t2.'.html';
  
		if ( $t2 == $page ) $pages .= '<span>'.$t2.'</span> ';
		else { if ($t2 == 1) { $plink = $config['http_home_url'].'music/downloadstopweek_admin.html';
					$pages .= '<a href="'.$plink.'">1</a> ';} else $pages .= '<a href="'.$plink.'">'.$t2.'</a> ';
				}
			$array[$t2] = 1;
		}

	$link = $config['http_home_url'].'music/downloadstopweek_admin-page-';
	$seo_mode = '.html';

	$npage = $page - 1;
	if ( isset($array[$npage]) ) {
    if ($npage ==1 ) {
      $linkdop = $config['http_home_url'].'music/downloadstopweek_admin';
