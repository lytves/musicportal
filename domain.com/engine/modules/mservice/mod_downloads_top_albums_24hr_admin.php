...
$nam_e1 = 'ТОП скачиваний музыкальных альбомов за сутки';
...
$metatags['title'] = 'ТОП скачиваний музыкальных альбомов за сутки '.$metapage.'на '.$config['home_title_short'];
$metatags['description'] = 'ТОП скачиваний музыкальных альбомов за сутки на '.$config['description'];
...
$mtitle = '<h3 class="tit">ТОП скачиваний музыкальных альбомов за сутки</h3>';

if ( $db->num_rows( ) == 0 ) $stop[] = 'Нет альбомов для этой страницы ТОП скачиваний музыкальных альбомов за сутки';
else {
  ...
  $mcontent .= '</tr></table></div>';
	...
	for ( $t = 0; $count_d > $t; $t++ ) {
		$t2 = $t + 1;
		$plink = $config['http_home_url'].'music/downloadstopalbums24hr_admin-page-'.$t2.'.html';
  
		if ( $t2 == $page ) $pages .= '<span>'.$t2.'</span> ';
		else { if ($t2 == 1) { $plink = $config['http_home_url'].'music/downloadstopalbums24hr_admin.html';
					$pages .= '<a href="'.$plink.'">1</a> ';} else $pages .= '<a href="'.$plink.'">'.$t2.'</a> ';
				}
			$array[$t2] = 1;
		}

	$link = $config['http_home_url'].'music/downloadstopalbums24hr_admin-page-';
	$seo_mode = '.html';

	$npage = $page - 1;
	if ( isset($array[$npage]) ) {
    if ($npage ==1 ) {
      $linkdop = $config['http_home_url'].'music/downloadstopalbums24hr_admin';
