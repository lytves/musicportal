...
$nam_e1 = 'Популярные поисковые запросы за неделю (по названию альбома)';
...
$metatags['title'] = 'Популярные поисковые запросы за неделю (по названию альбома) '.$metapage.'на '.$config['home_title_short'];
$metatags['description'] = 'Популярные поисковые запросы за неделю (по названию альбома) на '.$config['description'];
...
$db->query( "SELECT search_text, COUNT(search_text) AS cnt, (SELECT COUNT(DISTINCT search_text) FROM ".PREFIX."_mservice_search WHERE (search_type = '4' AND count != '0' AND time_php > '$toptime')) as cnt2 FROM ".PREFIX."_mservice_search WHERE (search_type = '4' AND count != '0' AND time_php > '$toptime') GROUP BY search_text HAVING cnt > 0 ORDER BY cnt DESC LIMIT ".$limit."," . $mscfg['track_page_lim'] );

$mtitle = '<h3 class="tit">Популярные поисковые запросы за неделю (по названию альбома)</h3>';

if ( $db->num_rows( ) == 0 ) $stop[] = 'Нет треков для этой страницы Популярных поисковых запросов за неделю (по названию альбома)';
else {
	$mcontent = <<<HTML
<table width="100%">
<tr>
<th width="85%" height="20">&nbsp;</th>
<th width="15%">Запросов</th>
</tr>
HTML;
	...
	while( $row = $db->get_row( ) ) {
		...
		$search_text_link = '<a href="'.$config['http_home_url'].'music/search-4-1-'.$search_text64.'.html" title="'.$row['search_text'].'">'.$search_text.'</a>';
	...
	for ( $t = 0; $count_d > $t; $t++ ) {
		$t2 = $t + 1;
		$plink = $config['http_home_url'].'music/popularsearchtitleweek-page-'.$t2.'.html';
  
		if ( $t2 == $page ) $pages .= '<span>'.$t2.'</span> ';
		else { if ($t2 == 1) { $plink = $config['http_home_url'].'music/popularsearchtitleweek.html';
					$pages .= '<a href="'.$plink.'">1</a> ';} else $pages .= '<a href="'.$plink.'">'.$t2.'</a> ';
				}
			$array[$t2] = 1;
		}

	$link = $config['http_home_url'].'music/popularsearchtitleweek-page-';
	$seo_mode = '.html';

	$npage = $page - 1;
	if ( isset($array[$npage]) ) { if ($npage ==1 ) { $linkdop = $config['http_home_url'].'music/popularsearchtitleweek';
