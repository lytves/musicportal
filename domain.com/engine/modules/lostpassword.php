...
if( $is_logged ) {
	...
	if( $row['lostid'] != "" AND $lostid != "" AND $row['lostid'] == $lostid ) {
		...
		if ($_GET['action'] == "ip") {
			...
			$salt = sha1( str_shuffle("abchefghjkmnpqrstuvwxyz0123456789".$stronghash) . microtime() );
			$new_pass = "";

			for($i = 0; $i < 11; $i ++) {
				$new_pass .= $salt{mt_rand( 0, 39 )};
			}
	...
	if( preg_match( "/[\||\'|\<|\>|\[|\]|\"|\!|\?|\$|\/|\\\|\&\~\*\{\+]/", $_POST['lostname'] ) OR !trim($_POST['lostname'])) {
		...
		if( $db->num_rows() > 0 ) {
			...
			$salt = sha1( str_shuffle("abchefghjkmnpqrstuvwxyz0123456789".$lostpass.$stronghash) . microtime() );
			$rand_lost = '';
			
			for($i = 0; $i < 15; $i ++) {
				$rand_lost .= $salt{mt_rand( 0, 39 )};
			}
