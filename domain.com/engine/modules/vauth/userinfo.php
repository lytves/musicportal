<?php
	...
	if ($row['vk_connected'] == 1) { $accounts = $accounts . '<a class="account_link vk_account" target="_blank" href="http://vk.com/id' . $row['vk_user_id'] .'">'.$vauth_text['profile_vk'].'</a> '; }
	if ($row['fb_connected'] == 1) { $accounts = $accounts . '<a class="account_link fb_account" target="_blank" href="http://facebook.com/' . $row['fb_user_id'] .'">'.$vauth_text['profile_fb'].'</a> '; }
	if ($row['tw_connected'] == 1) { $accounts = $accounts . '<a class="account_link tw_account" target="_blank" href="https://twitter.com/intent/user?user_id=' . $row['tw_user_id'] .'">'.$vauth_text['profile_tw'].'</a> '; }
	if ($row['fs_connected'] == 1) { $accounts = $accounts . '<a class="account_link fs_account" target="_blank" href="https://foursquare.com/user/' . $row['fs_user_id'] .'">'.$vauth_text['profile_fs'].'</a> '; }
	if ($row['od_connected'] == 1) { $accounts = $accounts . '<a class="account_link od_account" target="_blank" href="http://www.odnoklassniki.ru/profile/' . $row['od_user_id'] .'">'.$vauth_text['profile_od'].'</a> '; }
	if ($row['go_connected'] == 1) { $accounts = $accounts . '<a class="account_link go_account" target="_blank" href="https://plus.google.com/' . $row['go_user_id'] .'">'.$vauth_text['profile_go'].'</a> '; }
	if ($row['gh_connected'] == 1) { $accounts = $accounts . '<a class="account_link gh_account" target="_blank" href="https://github.com/' . $row['gh_username'] .'">'.$vauth_text['profile_gh'].'</a> '; }
	if ($row['ma_connected'] == 1) { $accounts = $accounts . '<a class="account_link ma_account" target="_blank" href="'.$row['ma_link'].'">'.$vauth_text['profile_ma'].'</a> '; }
	if ($row['ms_connected'] == 1) { $accounts = $accounts . '<a class="account_link ms_account" target="_blank" href="'.$row['ms_link'].$row['ms_user_id'].'/">'.$vauth_text['profile_ms'].'</a> '; }
	if ($row['in_connected'] == 1) { $accounts = $accounts . '<a class="account_link in_account" target="_blank" href="http://instagram.com/'.$row['name'].'">'.$vauth_text['profile_in'].'</a> '; }
	if ($row['ya_connected'] == 1) { $accounts = $accounts . '<a class="account_link ya_account" target="_blank" href="http://passport.yandex.ru/">'.$vauth_text['profile_ya'].'</a> '; }
