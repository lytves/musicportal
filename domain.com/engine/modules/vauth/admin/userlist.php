			...
			$where = array ();
				...
				if( ! empty( $_GET['site'] ) ) {
					...
					switch($sitelist) {
						...
						default:
							$where[] = ' ( fs_registered = \'1\' or go_registered = \'1\' or ma_registered = \'1\' or ms_registered = \'1\' or in_registered = \'1\' or gh_registered = \'1\' or od_registered = \'1\' or vk_registered = \'1\' or fb_registered = \'1\' or tw_registered = \'1\' or st_registered = \'1\' or ya_registered = \'1\' ) ';
						break;
					}
				...
				if( ! empty( $_GET['connected']) and $_GET['connected']== 'all')  {	
					...
					$where[] = '( fs_connected = \'1\' or go_connected = \'1\' or ma_connected = \'1\' or ms_connected = \'1\' or in_connected = \'1\' or gh_connected = \'1\' or od_connected = \'1\' or vk_connected = \'1\' or fb_connected = \'1\' or tw_connected = \'1\' or st_connected = \'1\' or ya_connected = \'1\' )';
				}
				if( ! empty( $_GET['reg'] ) ) {
...
while ( $row = $db->get_row( $connect_sql_login ) )	{
	...
	if($_GET['style']=='list') $tag .= '<span class="userlist_list"><a class="del_vauth_user_list" title="'.$vauth_text['admin_user_del'].$row['fullname'].$vauth_text['admin_user_del_site'].'" href="" uri="'.$admin_php_name.'?mod=vauth&page=users&del_user='.$row['user_id'].'"><b>&#935;</b></a><a target="_blank" href="/user/'.urlencode($row['name']).'">'.$row['name'].'</a></span>';
		...
		<a class="refresh_vauth_user" user_id="'.$row['user_id'].'" title="'.$vauth_text['admin_user_del_site_refresh'].''.$row['fullname'].$vauth_text['admin_user_del_site'].'" ><b>0</b></a>
		
		<a class="userlink" target="_blank" href="/user/'.urlencode($row['name']).'" title="'.$row['fullname'].'">
