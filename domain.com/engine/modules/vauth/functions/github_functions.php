...
if( ! class_exists( 'GhFunctions' ) )	{
	...
	class GhFunctions extends VAuthFunctions {
		...
		function oauth_data() {
			...
			if (empty($oauth['app_id'])) die('Unknown Application ID Github');
			if (empty($oauth['app_secret'])) die('Unknown Application ID Github');
			
			return $oauth;
			
		}	
	
		// ** Функция авторизации в github
		...
		// ** Функция получения информации пользователя из GitHub
