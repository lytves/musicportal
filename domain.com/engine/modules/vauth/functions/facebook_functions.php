...
if( ! class_exists( 'FbFunctions' ) )	{
	...
	class FbFunctions extends VAuthFunctions {
		...
		function oauth_data() {
			...
			$oauth['scope']				=	'email,user_status,user_birthday';
			...
			if (empty($oauth['app_id'])) die('Unknown Application ID Facebook');
			if (empty($oauth['app_secret'])) die('Unknown Application ID Facebook');
		...
		function get_oauth_info($oauth) {
			...
			///// вместо верхних закомментированных строк вставил эту
			$oauth['avatar'] 		=	'https://graph.facebook.com/'.$oauth['uid'].'/picture?type=large';
