...
if( ! class_exists( 'OdFunctions' ) )	{
	...
	class OdFunctions extends VAuthFunctions {
		...
		function oauth_data() {
			...
			$oauth['auth_url']			=	'http://www.odnoklassniki.ru/oauth/authorize?client_id='.$oauth['app_id'].'&response_type=code&scope=VALUABLE ACCESS;GET_EMAIL&redirect_uri='.$oauth['redirect_uri'];
		
			$oauth['group']				=	$vauth_config['odnoklassniki_user_group'];

			if (empty($oauth['group'])) $oauth['group'] = 4;
			if (empty($oauth['app_id'])) die('Unknown Application ID Odnoklassniki');
			if (empty($oauth['app_secret'])) die('Unknown Application ID Odnoklassniki');
		...
		function get_oauth_info($oauth) {
			...
			// получил от API саппорта разрешения на получение email
