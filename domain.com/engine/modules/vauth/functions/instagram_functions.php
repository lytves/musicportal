...
if( ! class_exists( 'InFunctions' ) )	{
	...
	class InFunctions extends VAuthFunctions {
		...
		function oauth_data() {
			...
			if (empty($oauth['app_id'])) die('Unknown Application ID instagram');
			if (empty($oauth['app_secret'])) die('Unknown Application ID instagram');
		...
		// ** Функция получения информации пользователя из instagram
