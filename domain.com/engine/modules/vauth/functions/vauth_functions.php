...
if( ! class_exists( 'VAuthFunctions' ) )	{
	...
	class VAuthFunctions {
		...
		function load_oauth_modules($auth_site) {
			...
			if (!$auth_site) die($this->conv_it('Unknown SocialNet For ID'));
...
// моя переработка  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$				
				if ($type == 2 || $type ==3) {
					...
					@unlink($image_dir.$image_name.$image_type);//на всякий случай
        include_once ENGINE_DIR.'/classes/thumb.class.php';
					
				$thumb = new thumbnail($avatar);
						
						if (empty($vauth_config['avatar_size'])) $vauth_config['avatar_size'] = 100;

						//$vauth_config['avatar_size']
						$avatar_size = $vauth_config['avatar_size'].'x'.$vauth_config['avatar_size'];
						if( $thumb->size_auto( "$avatar_size" ) ) {
							$thumb->jpeg_quality( 80 );
							//$thumb->jpeg_quality( $config['jpeg_quality'] );
							$thumb->save($image_dir.$image_name.$image_type);
							$foto_name = "foto_" . $id . $image_type;
						} else $foto_name = "noavatar.png";
