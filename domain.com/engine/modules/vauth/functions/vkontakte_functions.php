...
if( ! class_exists( 'VkFunctions' ) )	{
	...
	class VkFunctions extends VAuthFunctions {
		...
		function oauth_data() {
			...
			$oauth['auth_url']			=	'https://oauth.vk.com/authorize?client_id=' . $oauth['app_id'] .'&scope=friends,offline,email&redirect_uri='.$oauth['redirect_uri'].'&response_type=code';
			$oauth['group']				=	$vauth_config['vkontakte_user_group'];
		
			if (empty($oauth['group'])) $oauth['group'] = 4;
			if (empty($oauth['app_id'])) die('Unknown Application ID VK');
			if (empty($oauth['app_secret'])) die('НUnknown Application ID VK');
		...
		function vauth_auth($oauth) {
			...
			if ( !empty($auth_code) ) {
				...
				if (	!empty($userinfo->access_token) ) {
					...
					//добавил эту строку и выше в scope(права доступа) - email
					$oauth['email']		= $userinfo->email;
      ...
      //это старый ненужный код от автара
